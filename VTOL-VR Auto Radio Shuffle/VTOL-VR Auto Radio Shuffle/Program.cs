...
namespace VTOL_VR_Auto_Radio_Shuffle
{
    internal class Program
    {
        ...
        public static void renameFiles(List<string> mp3Files, List<int> numbers)
        {
            ...
            // First pass: Remove existing patterns
            for (int i = 0; i < mp3Files.Count; i++)
            {
                ...
                if (pattern.IsMatch(fileName))
                {
                    ...
                    else
                    {
                        ...
                        Console.WriteLine($"File {newFilePath} already exists, skipping renaming.");
                    }
                }
            }

            // Second pass: Add the new pattern
            for (int j = 0; j < mp3Files.Count; j++)
            {
